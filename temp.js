...
function playMove(pieceToMove, placeToMove) {
...
function isThereMate(otherColorNum,board) {
  ...
  var isMate = 1;
  var isPat = 1;
  if (isThereCheck(-1, -1, passToCheck) == -1) {
      isPat = -1;
      if (otherColorNum == 1) {
          for (var r = 0; r < 8; r++) {
              for (var c = 0; c < 8; c++) {
...
function getAvalibleSquaresR(pressedId, board) {
  ...
  if (
    ...
    if (document.getElementById(pressedId).style.backgroundColor != "red") {
      ...
      playMove(currentPiceMovingId, pressedId);
      isMarkedMove = -1;
    }
  }
}
  ...
  if (board[rowPressed][colPressed] == 1) {
    ...
    if (rowPressed - 1 > -1) {
      if (board[rowPressed - 1][colPressed] == 0) {
        if (
          isThereCheck(pressedId, rows * (rowPressed - 1) + colPressed, -1) == 1
        ) {
    ...
    if (rowPressed - 2 > -1) {
      if (
        ...
        if (
          isThereCheck(pressedId, rows * (rowPressed - 2) + colPressed, -1) == 1
        ) {
    ...
    while (end == 0) {
      if (rowPressed + i < 8 && RP == 0) {
        if (board[rowPressed + i][colPressed] == 0) {
          if (
            isThereCheck(pressedId, rows * (rowPressed + i) + colPressed, -1) ==
            1
          ) {
      ...
      if (colPressed - i > -1 && CM == 0) {
        if (board[rowPressed][colPressed - i] == 0) {
          if (
            isThereCheck(pressedId, rows * rowPressed + (colPressed - i), -1) ==
            1
          ) {
    ...
    if (
      ...
      if (
        isThereCheck(pressedId, rows * rowPressed + colPressed + 1, -1) == 1
      ) {
...
function endGameHelpEvaluateBoard(friendlyKingsquare,enemyKingSquare,endGameWeight){
  var evaluation=0;
  ...
  var eponentKingDstFromCentere=eponentKingDstToCentralCol+eponentKingDstToCentralRow;
  evaluation+= eponentKingDstFromCentere;
...
function evaluateBoard(board,isWhite) {
  ...
  if(1==1){
    if(isWhite==1){
      var endGameBust=endGameHelpEvaluateBoard(whiteKingsPlace,blackKingsPlace,8);
    }else{
      var endGameBust=-endGameHelpEvaluateBoard(blackKingsPlace,whiteKingsPlace,8);
    }
  }
  return matirialValue * 50+ positionalValue +endGameBust; 
}
...
function orderMoves(movesToOrder, board) {
  ...
  for (var i = 0; i < movesToOrder.length; i++) {
    ...
    if((isWhite==1&&board[rowToMove][colToMove]==-1)||(isWhite==-1 && board[rowToMove][colToMove]==1)){// were eating
      currentGuessValue+=  getPieceValue(board[rowToMove][colToMove])  - getPieceValue(board[rowPressed][colPressed]) ;
    }
    if((isWhite==1&&rowPressed==1&&piceType==1)||(isWhite==-1&&rowPressed==6&&piceType==-1)){//were premoting
      currentGuessValue += 15;
    }

    if (isWhite == 1 ) {// start of check if were moving to other pqwn
...
function minMaxAB(board, depth, alpha, beta, isWhite) {
  ...
  if (isWhite == 1) {
    ...
    for (var i = 0; i < possibleMoves.length; i++) {
      ...
      let score = minMaxAB(boardToPass, depth - 1, alpha, beta, -isWhite);
    ...
    for (var i = 0; i < possibleMoves.length; i++) {
      ...
      let score = minMaxAB(boardToPass, depth - 1, alpha, beta, 1); // להעביר את הלוח אחרי השיחוק
...
function SearchAllCaptures(board,depth,alpha,beta,isWhite){
  ...
  for(var i=0; i<cuptureMoves.length; i++){
    ...
    evaluation=-SearchAllCaptures(visuMoveBoard,depth-1,-beta,-alpha,-isWhite);
...
function enemyPlay() {
  ...
  for (var i = 0; i < possibleMoves.length; i++) {
    ...
    currentScore = minMaxAB(boardToPass, 3, -Infinity, +Infinity, -1); //,-Infinity,+Infinity
  ...
  playMove(possibleMoves[bestIritation][0], possibleMoves[bestIritation][1]);
} 
